document.addEventListener('DOMContentLoaded', function () {
  ...
  function matchesFilters(r) {
    ...
    if (state.area && r.district !== state.area) return false;

    if (state.searchEventType && !r.eventTypes.includes(state.searchEventType)) return false;

    if (state.guests) {
      const g = Number(state.guests);
      if (!Number.isNaN(g) && g > r.capacityMax) return false;
    }

    for (const tag of state.quickTags) {
      if (!applyQuickFilterCondition(r, tag)) return false;
    }
    ...
    if (state.capacity) {
      ...
      if (state.capacity === '<50' && !(max < 50)) return false;
      if (state.capacity === '50-150' && !(max >= 50 && min <= 150)) return false;
      if (state.capacity === '150-300' && !(max >= 150 && min <= 300)) return false;
      if (state.capacity === '300+' && !(max >= 300)) return false;
    }

    if (state.rating > 0 && r.rating < state.rating) return false;

    if (state.status && r.availability !== state.status) return false;

    if (r.pricePerGuest > state.priceMax) return false;
...
<article class="group bg-white rounded-[28px] border border-[#E5E7EB] shadow-sm hover:shadow-2xl hover:-translate-y-2 hover:shadow-[#D4AF37]/20 hover:border-[#D4AF37]/50 transition-all duration-500 ease-out relative overflow-hidden">
  ...
  <div class="p-6 relative z-10">
    ...
    <div class="flex items-center gap-3">
      ...
      <button type="button" class="flex-1 px-4 py-2.5 bg-[#0B1120] hover:bg-[#020617] text-white text-sm font-semibold rounded-xl border-b-2 border-[#D4AF37] transition-all hover:scale-[1.02] active:scale-95 hover:shadow-lg hover:shadow-[#D4AF37]/20" data-action="details">
        View details
      </button>
  ...
  document.addEventListener('click', (e) => {
    ...
    if (action === 'favorite') {
      ...
      alert('View details (demo – detail page will be implemented later).');
    } else if (action === 'book') {
  ...
  if (mobileBackdrop) mobileBackdrop.addEventListener('click', closeSheet);
